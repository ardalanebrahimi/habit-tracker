...
@Component({
    ...
    <div class="subscription-management">
      ...
      <div class="token-packs">
        ...
        <p class="token-description">
          Need more tokens? Purchase token packs for instant access to premium features.
        </p>
      ...
      <div class="referral-section">
        ...
        <div class="referral-card">
          <div class="referral-info">
            <p>Share your referral code and earn <strong>2 tokens</strong> for each friend who joins!</p>
            <div class="referral-code" *ngIf="referralCode">
              <label>Your referral code:</label>
              <div class="code-display">
                <span class="code">{{ referralCode }}</span>
                <button class="copy-btn" (click)="copyReferralCode()">📋 Copy</button>
              </div>
            </div>
          </div>
        </div>
  ...
  styles: [`
    ...
    .status-card, .plan-card, .pack-card, .referral-card {
      background: white;
    ...
    .plan-card.recommended, .pack-card.popular {
      border-color: #3b82f6;
      position: relative;
      transform: scale(1.02);
    }

    .plan-badge, .pack-badge {
      position: absolute;
      top: -8px;
      left: 50%;
...
export class SubscriptionManagementComponent implements OnInit {
  ...
  async subscribeToPlan(plan: SubscriptionPlan): Promise<void> {
    try {
      ...
      alert('Subscription feature is not yet available. Please try again later.');
    }
  }
  ...
  async purchaseTokenPack(packId: string): Promise<void> {
    try {
      ...
      alert('Token purchase feature is not yet available. Please try again later.');
    }
  }
